...
var tipTimer
var touchPreventDefaultIdList = [] // 阻止页面滑动的DOM元素ID集合
export default {
  install: function (Vue, options) {
    // 当页面弹出指定内容时，阻止页面的滑动
    document.addEventListener('touchmove', function (event) {
      if (touchPreventDefaultIdList.length > 0) {
        for (var p = 0; p < touchPreventDefaultIdList.length; p++) {
          if (document.getElementById(touchPreventDefaultIdList[p])) {
    ...
    Vue.prototype.$addTouchPreventDomId = function (idVal) {
      if (!touchPreventDefaultIdList.indexOf(idVal) >= 0) {
        touchPreventDefaultIdList.push(idVal)
      }
    }
    // toast弹出层
    ...
    // 提示信息弹出层
    Vue.prototype.$tip = function (TipPopComponent) {
      if (document.getElementById('tip-message-box')) {
        return false
      }
      var tipShadeElem = document.createElement('div')
      tipShadeElem.id = 'tip-message-box-shade'
      tipShadeElem.style.position = 'fixed'
      tipShadeElem.style.width = '100vw'
      tipShadeElem.style.height = '100vh'
      tipShadeElem.style.top = 0
      tipShadeElem.style.left = 0
      tipShadeElem.style.transition = 'opacity 0.3s ease 0s'
      tipShadeElem.style.opacity = 0
      tipShadeElem.style.zIndex = 99999998
      tipShadeElem.style.backgroundColor = 'rgba(0, 0, 0, 0.2)'
      document.body.appendChild(tipShadeElem)
      setTimeout(() => {
        tipShadeElem.style.opacity = 1
      }, 10)
      tipShadeElem.onclick = function () {
        if (document.getElementById('tip-message-box')) {
          if (tipTimer) {
            clearTimeout(tipTimer)
          }
          document.getElementById('tip-message-box-shade').style.opacity = 0
          document.getElementById('tip-message-box').style.opacity = 0
          tipTimer = setTimeout(() => {
            document.body.removeChild(document.getElementById('tip-message-box-shade'))
            document.body.removeChild(document.getElementById('tip-message-box'))
          }, 310)
        }
      }
      ...
      tipElem.style.whiteSpace = 'nowrap'
      tipElem.style.display = 'inline-block'
      tipElem.style.transition = 'opacity 0.3s ease 0s'
      tipElem.style.opacity = 0
      tipElem.style.fontSize = '0.8rem'
      tipElem.style.zIndex = 99999999
      tipElem.style.color = 'rgba(58, 58, 58, 1)'
    ...
    // 关闭tip弹窗
    Vue.prototype.$tip_close = function () {
      var tipClosePromise = new Promise((resolve) => {
        if (document.getElementById('tip-message-box')) {
          if (tipTimer) {
            clearTimeout(tipTimer)
          }
    ...
    Vue.prototype.$picker = function (context, option) {
      ...
      var selected = [] // 保存选中的值
      var params = option || {}
      var root = params.root || null // 生产滑轮选择的根容器
      ...
      var domRoot = root || document.body
      ...
      if (root === null) {
        ...
        domRoot.appendChild(pickerShade)
        setTimeout(() => {
      ...
      pickerElem.style.position = root === null ? 'fixed' : 'relative'
      ...
      domRoot.appendChild(pickerElem)
      setTimeout(() => {
      ...
      if (root === null) {
        pickerShade.onclick = function () {
          ...
          pickerElem.style.transform = 'translateY(110%)'
          setTimeout(() => {
            domRoot.removeChild(this)
            domRoot.removeChild(pickerElem)
          }, 400)
      ...
      if (root === null) {
        ...
        if (debug) {
          ...
          pickerTitleLeftElem.style.backgroundImage = 'url(' + require('@/plugins/bulb-dark.png') + ')'
          ...
          pickerTitleLeftElem.onclick = function () {
            ...
            for (let h = 0; h < helpDataArr.length; h++) {
              ...
              ;(function (target, helpInfo) {
                target.onclick = function () {
                  showPickerInfoPop(helpInfo.title, helpInfo.doc, false, root !== null)
                }
            ...
            showPickerInfoPop('picker 使用帮助文档', helpListWrapElem, true, root !== null)
          }
        }
        ...
        pickerTitleRightElem.style.backgroundImage = 'url(' + require('@/plugins/close-dark.png') + ')'
        ...
        pickerTitleRightElem.onclick = function () {
          ...
          setTimeout(() => {
            if (root === null) {
              domRoot.removeChild(pickerShade)
            }
            domRoot.removeChild(pickerElem)
          }, 400)
          selectedFinishFn(selected, arg)
      ...
      pickerWheelWrapElem.style.width = '100%'
      ...
      if (type) {
        ...
        if (type.toLowerCase() === 'yyyy-mm-dd' || type.toLowerCase() === 'yyyy-mm') {
          ...
          if (minVal && minVal !== '~') {
            ...
            monthMin = Number(new Date().getMonth()) + 1
          }
          let monthMax = 12
          if (maxVal && maxVal !== '~') {
            monthMax = 12
          ...
          colCallBack[0] = function (pickerSelected, rowIndex) {
            if (maxVal && maxVal === '~') {
              ...
              let changeMonthData = []
              for (let m = monthMin; m <= changeMonthMax; m++) {
                changeMonthData.push({
                  val: m,
                  unit: '月'
                })
              }
              cols[1] = changeMonthData
              ...
              if (type.toLowerCase() === 'yyyy-mm-dd') {
                ...
                if (minVal && minVal !== '~') {
                  ...
                  dayMin = Number(new Date().getDate())
                }
                let changeDayData = []
                for (let d = dayMin; d <= changeDayMax; d++) {
                  changeDayData.push({
                    val: d,
                    unit: '日'
                  })
                }
                cols[2] = changeDayData
      ...
      if (cols.length > 0) {
        ...
        for (let row = 0; row < cols.length; row++) {
          if (cols[row] && isArr(cols[row]) && cols[row].length > 0) {
            ...
            ;(function (pickerDataWheelWrapElem, pickerUnitWheelWrapElem, row) {
              ...
              pickerDataWheelWrapElem.addEventListener('touchend', function () {
                ...
                if (translateToY < minY + colLineHeightToPx) {
                  // 选中最后一项
                  let selectedItemUnit = cols[row][cols[row].length - 1].unit || cols[row][0].unit || null
                  ...
                  selected[row] = {
                    val: cols[row][cols[row].length - 1].val || '',
                    display: cols[row][cols[row].length - 1].display || '',
                    unit: cols[row][cols[row].length - 1].unit || null
                  }
                  selectedFn(selected, arg)
                } else if (translateToY > maxY) {
                  // 选中最前一项
                  let selectedItemUnit = cols[row][0].unit || null
                  ...
                  selected[row] = {
                    val: cols[row][0].val || '',
                    display: cols[row][0].display || '',
                    unit: cols[row][0].unit || null
                  }
                  selectedFn(selected, arg)
                } else {
                  ...
                  if (bryond < colLineHeightToPx / 2) {
                    let selectedItemUnit = cols[row][overItemCount].unit || cols[row][0].unit || null
                    ...
                    selected[row] = {
                      val: cols[row][overItemCount].val || '',
                      display: cols[row][overItemCount].display || '',
                      unit: cols[row][overItemCount].unit || null
                    }
                    selectedFn(selected, arg)
                  } else {
                    let selectedItemUnit = cols[row][overItemCount + 1].unit || cols[row][0].unit || null
                    ...
                    selected[row] = {
                      val: cols[row][overItemCount + 1].val || '',
                      display: cols[row][overItemCount + 1].display || '',
                      unit: cols[row][overItemCount + 1].unit || null
                    }
                    selectedFn(selected, arg)
                  }
                }
            ...
            })(pickerDataWheelWrapElem, pickerUnitWheelWrapElem, row)
          }
        }
        return {
          initVal: () => {
        ...
        var wheelHeightToPx = 0
        if (wheelHeight.indexOf('rem') > 0) {
          wheelHeightToPx = Number(wheelHeight.match(/[-?\d.?]+/g)[0]) * 16
        } else {
          wheelHeightToPx = Number(wheelHeight.match(/[-?\d.?]+/g)[0])
        }
        pickerWheelWrapElem.innerHTML = ''
        ...
        noColsDataTipElem.style.lineHeight = wheelHeightToPx + 'px'
        ...
        pickerColsTipA.onclick = function () {
          showPickerInfoPop('属性 cols 说明文档', pickerColsDoc, false, root !== null)
        }
        ...
        pickerNoColTipA.onclick = function () {
          showPickerInfoPop('属性 type 说明文档', pickerTypeDoc, false, root !== null)
        }
...
function showPickerInfoPop (title, contentHtml, isNode, isRoot) {
